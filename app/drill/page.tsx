...
export default async function Drill({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const p: DrillParams = {
    ...
    pt: one(sp.pt), pta: one(sp.pta), fstage: one(sp.fstage), prog: one(sp.prog), cb: one(sp.cb), has: one(sp.has),
  };
  ...
  const csvQs = (() => {
    ...
    const single: [string, string | null | undefined][] = [
      ...
      ["pt", p.pt], ["pta", p.pta], ["fstage", p.fstage], ["prog", p.prog], ["cb", p.cb], ["has", p.has],
    ];
