...
export default async function Drill({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const p: DrillParams = {
    ...
    id: one(sp.id), name: one(sp.name), phone: one(sp.phone), tg: one(sp.tg), tag: one(sp.tag),
  };
  ...
  const csvQs = (() => {
    ...
    const single: [string, string | null | undefined][] = [
      ...
      ["cprog", p.cprog], ["sprog", p.sprog], ["id", p.id], ["name", p.name], ["phone", p.phone], ["tg", p.tg], ["tag", p.tag],
    ];
