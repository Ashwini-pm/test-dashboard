...
export default async function Drill({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const p: DrillParams = {
    ...
    pt: one(sp.pt), pta: one(sp.pta), fstage: one(sp.fstage), prog: one(sp.prog), cb: one(sp.cb), has: one(sp.has), xyz: one(sp.xyz),
  };
  ...
  const csvQs = (() => {
    ...
    const single: [string, string | null | undefined][] = [
      ...
      ["pt", p.pt], ["pta", p.pta], ["fstage", p.fstage], ["prog", p.prog], ["cb", p.cb], ["has", p.has], ["xyz", p.xyz],
    ];
      ...
      <section className="grid mb">
        <div className="card">
          ...
          <form method="GET" action="/drill">
            ...
            {p.pt && <input type="hidden" name="pt" value={p.pt} />}
            {p.pta && <input type="hidden" name="pta" value={p.pta} />}
            {p.fstage && <input type="hidden" name="fstage" value={p.fstage} />}
            {p.prog && <input type="hidden" name="prog" value={p.prog} />}
            {p.cb && <input type="hidden" name="cb" value={p.cb} />}
            {p.has && <input type="hidden" name="has" value={p.has} />}
            {p.xyz && <input type="hidden" name="xyz" value={p.xyz} />}
